...
export const ConfiguracionComboModal = ({
  open,
  onClose,
  combo,
  onConfigSaved
  ...
  const [itemsDetalles, setItemsDetalles] = useState([]); // Store details for chips
  ...
  const cargarItemsPorProveedor = async (proveedorId) => {
    try {
      ...
      // Update details if any of these items are already selected
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      if (combo?.id) {
        await dashboardSalidasService.actualizarConfig(combo.id, data);
      } else {
        await dashboardSalidasService.crearConfig(data);
      }
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      <DialogTitle sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Typography variant="h6" fontWeight={700}>Configurar Combo de Materiales</Typography>
