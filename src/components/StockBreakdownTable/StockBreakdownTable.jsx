...
export const StockBreakdownTable = ({ dataset, loading }) => {
  ...
  const filteredData = useMemo(() => {
    ...
    return dataset.filter(row => 
      (row.item?.descripcion?.toLowerCase().includes(term)) ||
      (row.proveedor?.nombre?.toLowerCase().includes(term)) ||
      (row.partida?.numeroPartida?.toLowerCase().includes(term))
    );
  }, [dataset, searchTerm]);
  ...
  if (!dataset || dataset.length === 0) {
    return (
      <Paper sx={{ p: 4, textAlign: 'center', borderRadius: 3 }}>
        <Typography color="text.secondary">No se encontró stock disponible.</Typography>
      </Paper>
    );
  }
        ...
        <Table sx={{ minWidth: 650 }}>
          <TableHead sx={{ bgcolor: '#f8f9fa' }}>
            <TableRow>
              ...
              <TableCell sx={{ fontWeight: 700 }}>Dueño (Proveedor)</TableCell>
          ...
          <TableBody>
            {filteredData.map((row) => (
              <TableRow key={row.id} hover>
                <TableCell sx={{ fontWeight: 500 }}>{row.item?.descripcion || 'N/A'}</TableCell>
                <TableCell>{row.proveedor?.nombre || 'N/A'}</TableCell>
                <TableCell>
                  <Chip 
                    label={row.item?.categoria || 'Sin Cat.'} 
                    size="small" 
                    variant="outlined"
                    sx={{ fontSize: '0.7rem' }}
                  />
                </TableCell>
                <TableCell>{row.partida?.numeroPartida || 'N/A'}</TableCell>
