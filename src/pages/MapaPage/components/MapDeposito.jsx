...
const MapDeposito = () => {
  ...
  // Calcular movimientos necesarios para llegar de real a ideal
  const movimientosNecesarios = React.useMemo(() => {
    const movimientos = [];
    ...
    posiciones.forEach(posicion => {
      ...
      // Solo procesar posiciones que tienen categoría ideal definida
      if (categoriaIdeal) {
        if (!categoriaReal || categoriaReal !== categoriaIdeal) {
          // Necesita movimiento
          movimientos.push({
            rack: posicion.rack,
            fila: posicion.fila,
            AB: posicion.AB,
            categoriaActual: categoriaReal || 'Vacía',
            categoriaIdeal: categoriaIdeal,
            pesoActual: categoriaReal ? (pesosPorCategoria[categoriaReal] || 0) : 0,
            pesoIdeal: pesosPorCategoria[categoriaIdeal] || 0,
            diferenciaPeso: (pesosPorCategoria[categoriaIdeal] || 0) - (categoriaReal ? (pesosPorCategoria[categoriaReal] || 0) : 0)
          });
        }
      }
    });
    
    // Ordenar por proximidad de racks (menor diferencia entre racks)
    movimientos.sort((a, b) => {
      // Calcular distancia promedio de racks para cada movimiento
      const distanciaA = Math.abs(a.rack - 10); // Centro aproximado
      const distanciaB = Math.abs(b.rack - 10);
      return distanciaA - distanciaB;
    });
    
    return movimientos;
  }, [posiciones, categoriasReales, categoriasIdeales, pesosPorCategoria]);
  ...
  return (
    <Box sx={{ width: '100%', px: 2 }}>
      ...
      {/* Lista de Movimientos Necesarios - Solo en vista ideal */}
      {currentView === 1 && movimientosNecesarios.length > 0 && (
        <Box sx={{ mt: 4, mb: 3 }}>
          <Card sx={{ backgroundColor: 'var(--color-surface)', border: '1px solid var(--color-border)' }}>
            <CardContent>
              <Typography variant="h6" sx={{ mb: 3, fontWeight: 600, textAlign: 'center', color: 'var(--color-text-primary)' }}>
                Movimientos Necesarios para Vista Ideal
              </Typography>
              
              <Typography variant="body2" sx={{ mb: 3, textAlign: 'center', color: 'var(--color-text-secondary)' }}>
                {movimientosNecesarios.length} movimientos necesarios (ordenados por proximidad de racks)
              </Typography>
              
              <Box sx={{ maxHeight: 400, overflowY: 'auto' }}>
                {movimientosNecesarios.map((movimiento, index) => (
                  <Box
                    key={`${movimiento.rack}-${movimiento.fila}-${movimiento.AB}`}
                    sx={{
                      display: 'flex',
                      alignItems: 'center',
                      gap: 2,
                  ...
                  >
                    ...
                    <Box sx={{ flex: 1, minWidth: 0 }}>
                      <Typography variant="body2" sx={{ fontWeight: 600, color: 'var(--color-text-primary)', mb: 0.5 }}>
                        Rack {movimiento.rack} - Fila {movimiento.fila} - {movimiento.AB}
                      </Typography>
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          ...
                          <Box
                            sx={{
                              width: 12,
                              height: 12,
                              backgroundColor: coloresPorCategoria[movimiento.categoriaActual] || 'var(--color-divider)',
                              borderRadius: 'var(--border-radius-sm)',
                              border: '1px solid var(--color-border)'
                            }}
                          />
                          <Typography variant="body2" sx={{ fontWeight: 500 }}>
                            {movimiento.categoriaActual}
                          </Typography>
                          {movimiento.pesoActual > 0 && (
                            <Typography variant="caption" sx={{ color: 'var(--color-text-secondary)' }}>
                              ({movimiento.pesoActual} kg)
                            </Typography>
                          )}
                        </Box>
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          ...
                          <Box
                            sx={{
                              width: 12,
                              height: 12,
                              backgroundColor: coloresPorCategoria[movimiento.categoriaIdeal] || 'var(--color-divider)',
                              borderRadius: 'var(--border-radius-sm)',
                              border: '1px solid var(--color-border)'
                            }}
                          />
                          <Typography variant="body2" sx={{ fontWeight: 500, color: 'var(--color-primary)' }}>
                            {movimiento.categoriaIdeal}
                          </Typography>
                          <Typography variant="caption" sx={{ color: 'var(--color-text-secondary)' }}>
                            ({movimiento.pesoIdeal} kg)
                          </Typography>
                        </Box>
                      </Box>
                      
                      {/* Diferencia de peso */}
                      {movimiento.diferenciaPeso !== 0 && (
                        <Typography 
                          variant="caption" 
                          sx={{ 
                            color: movimiento.diferenciaPeso > 0 ? 'var(--color-success)' : 'var(--color-warning)',
                            fontWeight: 600,
                            mt: 0.5,
                            display: 'block'
                          }}
                        >
                          {movimiento.diferenciaPeso > 0 ? '+' : ''}{movimiento.diferenciaPeso} kg diferencia
                        </Typography>
                      )}
                    </Box>
                  </Box>
              ...
              >
                <Typography variant="body2" sx={{ fontWeight: 600, color: 'var(--color-text-primary)', textAlign: 'center' }}>
                  Total: {movimientosNecesarios.length} movimientos necesarios para alcanzar la disposición ideal
                </Typography>
              </Box>
            </CardContent>
          </Card>
